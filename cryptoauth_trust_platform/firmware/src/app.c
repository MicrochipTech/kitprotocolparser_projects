    ...
    switch(appData.state)
    {
        ...
        case APP_STATE_MAIN_TASK:
            ...
            else if( appData.hidDataReceived )
            {
                ...
                usb_hid_report_out_callback(&receiveDataBuffer[0]);
                if (g_usb_message_received == true)
                {
                    LED_Toggle();
                }
                appData.hidDataReceived = false;
            }
            else if (appData.hidDataTransmitted && response_available)
            {
                // Get the response length
                usb_report_length = min(64, response_len);

                appData.hidDataTransmitted = false;
                memset(appData.transmitDataBuffer , 0, 64);

                // copy command response into usb transmit buffer
                                    ...
                                    &appData.txTransferHandle, appData.transmitDataBuffer, 64)))
                {
                    ...
                    if (response_len <= 0)
                    {
                        ...
                        memset(&g_usb_buffer[0], 0, KIT_MESSAGE_SIZE_MAX);

                        current_response_location = 0;
                        usb_report_length = 0;
                        appData.hidDataReceived = false;
                        LED_Toggle();
